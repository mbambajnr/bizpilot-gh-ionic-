...
| P2 | Evidence and approvals | Procurement documents are supported; approval delegation, threshold routing, and configurable per-document-type approval thresholds (expenses and supplier bills) are implemented; expense and payment evidence is mostly reference-based. | Add private attachments, reviewer comments, escalation timers, and retention policies; extend configurable thresholds to purchase orders and transfers. |
