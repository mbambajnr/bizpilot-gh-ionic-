...
import { selectProcessableQuotes, defaultOrderTypeAutoReserves, selectQuotesForAutoReserve, describeConversionBlock } from '../../src/utils/orderProcessing';
...
function EnterpriseQuotationsView({ initialQuotationId }: { initialQuotationId: string }) {
  const { state, currentUser, hasPermission, addQuotation, registerQuotationProspect, convertQuotationToSale, addQuotationClientPo, removeQuotationClientPo, setQuotationHold } = useBusiness();
  ...
  return <EnterpriseShell active="Quotations"><div className="page-content quotations-native-page">
    ...
    {view === 'process' ? <ProcessOrdersPanel state={state} currency={currency} /> : <section className="quotation-workspace"><div className="quotation-list-panel"><div className="quotation-toolbar"><label><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search quotation, customer, or item" /></label><span>{filtered.length} documents</span></div><div className="quotation-table-wrap"><table className="quotation-table"><thead><tr><th>Quotation</th><th>Customer</th><th>Validity</th><th>Value</th><th>Status</th><th /></tr></thead><tbody>{filtered.map((quotation) => { const display = selectQuotationStatusDisplay(quotation); return <tr className={selected?.id === quotation.id ? 'quotation-row quotation-row--selected' : 'quotation-row'} onClick={() => setSelectedId(quotation.id)} key={quotation.id}><td><strong>{quotation.quotationNumber}</strong><span>{quotation.items.length} lines · {formatRelativeDate(quotation.createdAt)}</span></td><td><strong>{quotation.customerName}</strong><span>{quotation.clientId}</span></td><td>{quotation.validUntil ? new Date(quotation.validUntil).toLocaleDateString() : 'Not set'}</td><td><strong>{formatCurrency(quotation.netReceivableAmount ?? quotation.totalAmount, currency)}</strong></td><td><span className={`quotation-status quotation-status--${display.tone}`}>{display.label}</span></td><td><ChevronRight size={15} /></td></tr>;})}</tbody></table>{!filtered.length ? <QuotationEmpty canCreate={canCreate && view === 'active'} onCreate={() => setComposerOpen(true)} /> : null}</div></div>
      <QuotationInspector quotation={selected} state={state} currency={currency} now={pageOpenedAt} canConvert={canConvert} canRegisterCustomers={canRegisterCustomers} canManageClientPo={canCreate || canConvert} canPrint={canPrint} canExport={canExport} canHoldOrders={canConvert} onConvert={setConversionTarget} onRegister={setRegistrationTarget} onUploadClientPo={attachClientPo} onOpenClientPo={openClientPo} onRemoveClientPo={removeClientPo} onSetHold={(quote, onHold) => { const result = setQuotationHold({ quotationId: quote.id, onHold }); setMessage(result.ok ? (onHold ? 'Order placed on hold.' : 'Order hold released.') : result.message); }} />
    </section>}
...
function QuotationInspector({ quotation, state, currency, now, canConvert, canRegisterCustomers, canManageClientPo, canPrint, canExport, canHoldOrders, onConvert, onRegister, onUploadClientPo, onOpenClientPo, onRemoveClientPo, onSetHold }: { quotation?: Quotation; state: ReturnType<typeof useBusiness>['state']; currency: string; now: number; canConvert: boolean; canRegisterCustomers: boolean; canManageClientPo: boolean; canPrint: boolean; canExport: boolean; canHoldOrders: boolean; onConvert: (quotation: Quotation) => void; onRegister: (quotation: Quotation) => void; onUploadClientPo: (quotation: Quotation, file: File, poNumber: string) => Promise<boolean>; onOpenClientPo: (quotation: Quotation, document: ClientPurchaseOrderDocument) => Promise<void>; onRemoveClientPo: (quotation: Quotation, document: ClientPurchaseOrderDocument) => Promise<boolean>; onSetHold: (quotation: Quotation, onHold: boolean) => void }) {
  ...
  const conversionBlock = convertible ? describeConversionBlock(state, quotation) : null;
  const showHoldControls = canHoldOrders && convertible;
  return <aside className="quotation-inspector"><div className="quotation-inspector-head"><div><p className="eyebrow">Customer quotation</p><h2>{quotation.quotationNumber}</h2><span>{quotation.customerName} · {quotation.clientId}</span></div><span className={`quotation-status quotation-status--${status.tone}`}>{status.label}</span></div>
    ...
    {quotation.onHold ? <div className="quotation-order-hold quotation-order-hold--held"><PauseCircle size={16} /><div><strong>Order on hold</strong><small>Parked on entry — it cannot be invoiced until the hold is released.</small></div>{canHoldOrders ? <button className="secondary-button" type="button" onClick={() => onSetHold(quotation, false)}><PlayCircle size={14} /> Release hold</button> : null}</div> : showHoldControls ? <div className="quotation-order-hold"><button className="text-button" type="button" onClick={() => onSetHold(quotation, true)}><PauseCircle size={14} /> Put order on hold</button></div> : null}
...
function ClientPurchaseOrderPanel({ quotation, canManage, onUpload, onOpen, onRemove }: { quotation: Quotation; canManage: boolean; onUpload: (quotation: Quotation, file: File, poNumber: string) => Promise<boolean>; onOpen: (quotation: Quotation, document: ClientPurchaseOrderDocument) => Promise<void>; onRemove: (quotation: Quotation, document: ClientPurchaseOrderDocument) => Promise<boolean> }) {
  ...
  return <section className="quotation-po-panel"><div className="quotation-section-heading"><strong>Client purchase order</strong><span>{documents.length ? `${documents.length} attached` : 'Approval evidence'}</span></div>
    ...
    {canManage && storageConfigured === false ? <div className="quotation-po-unavailable"><ShieldAlert size={15} /><span><strong>Document storage isn&apos;t set up</strong><small>An administrator needs to set <code>SUPABASE_SERVICE_ROLE_KEY</code> and create the private document bucket (<code>SUPABASE_DOCUMENTS_BUCKET</code>, defaults to <code>documents</code>) before client POs can be attached.</small></span></div> : null}
