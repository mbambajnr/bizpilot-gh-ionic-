-- Allow inventory imports to bootstrap referenced stock locations when cloud setup is incomplete.
  ...
  loop
    ...
    if resolved_location_id is null then
      insert into public.business_locations (
        business_id, location_code, name, type, address, manager_name, is_default, is_active
      )
      values (
        target_business_id,
        nullif(trim(location_record.location_code), ''),
        trim(location_record.name),
        coalesce(nullif(trim(location_record.type), ''), 'store'),
        nullif(trim(coalesce(location_record.address, '')), ''),
        nullif(trim(coalesce(location_record.manager_name, '')), ''),
        coalesce(location_record.is_default, false)
          and not exists (
            select 1 from public.business_locations
            where business_id = target_business_id and is_default = true
          ),
        coalesce(location_record.is_active, true)
      )
